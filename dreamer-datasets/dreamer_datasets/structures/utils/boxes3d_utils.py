...
def crop_corners3d(corners3d, z_b=0.5, threshold = 0):
    # keep = []
    for i in range(len(corners3d)):
        back_cam_mask = corners3d[i, :, 2] < threshold
        # if back_cam_mask.all():
        #     continue
        # keep.append(i)
...
def xywhr2xyxyr(bev_boxes):
    # bev_boxes: (N, 5)
    assert bev_boxes.shape[1] == 5
    if isinstance(bev_boxes, torch.Tensor):
        new_bev_boxes = torch.zeros_like(bev_boxes)
    else:
        new_bev_boxes = np.zeros(bev_boxes.shape, dtype=bev_boxes.dtype)
    half_w = bev_boxes[:, 2] / 2
    half_h = bev_boxes[:, 3] / 2
    new_bev_boxes[:, 0] = bev_boxes[:, 0] - half_w
    new_bev_boxes[:, 1] = bev_boxes[:, 1] - half_h
    new_bev_boxes[:, 2] = bev_boxes[:, 0] + half_w
    new_bev_boxes[:, 3] = bev_boxes[:, 1] + half_h
    new_bev_boxes[:, 4] = bev_boxes[:, 4]
    return new_bev_boxes
...
def overlaps(
    ...
    if method.startswith('minmax'):
        ...
        if method == 'depth_nearest_bev':
            ...
            from giga_datasets.ops import boxes_overlap_bev

            boxes_a_bev = xywhr2xyxyr(boxes3d_to_bev_boxes2d(boxes3d_a, bev_axis))
            boxes_b_bev = xywhr2xyxyr(boxes3d_to_bev_boxes2d(boxes3d_b, bev_axis))
            if isinstance(boxes3d_a, torch.Tensor):
                overlap_bev = boxes_overlap_bev(boxes_a_bev, boxes_b_bev)  # (n, m)
            else:
                overlap_bev = boxes_overlap_bev(torch.from_numpy(boxes_a_bev), torch.from_numpy(boxes_b_bev))  # (n, m)
                overlap_bev = overlap_bev.numpy()
        else:
            assert False
        # compute ios
        ...
        boxes_b_bev = xywhr2xyxyr(boxes3d_to_bev_boxes2d(boxes3d_b, bev_axis))
        if isinstance(boxes3d_a, torch.Tensor):
            overlap = boxes_overlap_bev(boxes_a_bev, boxes_b_bev)  # (n, m)
        else:
            overlap = boxes_overlap_bev(torch.from_numpy(boxes_a_bev), torch.from_numpy(boxes_b_bev))  # (n, m)
            overlap = overlap.numpy()
        if mode is None:
            return overlap
        if mode in ('iou', 'ioa'):
            size_a = boxes_utils.get_size(boxes_a_bev)
        if mode in ('iou', 'iob'):
            size_b = boxes_utils.get_size(boxes_b_bev)
        if mode == 'iou':  # intersection over union
