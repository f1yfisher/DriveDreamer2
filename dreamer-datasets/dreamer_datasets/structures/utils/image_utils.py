...
def get_image_size(src_size, dst_size, mode, max_size=None, multiple=None):
    ...
    if mode in ('long', 'short', 'height', 'width'):
        ...
        if mode == 'outer_fit':
            if float(dst_height) / height > float(dst_width) / width:
                dst_height = int(float(dst_width) / width * height)
            else:
                dst_width = int(float(dst_height) / height * width)
        elif mode == 'inner_fit':
            if float(dst_height) / height < float(dst_width) / width:
                dst_height = int(float(dst_width) / width * height)
            else:
    ...
    if max_size is not None and max(dst_height, dst_width) > max_size:
        if dst_height > dst_width:
            dst_width = int(float(max_size) / dst_height * dst_width)
            dst_height = max_size
        else:
            dst_height = int(float(max_size) / dst_width * dst_height)
            dst_width = max_size
    if multiple is not None:
        dst_height = dst_height - dst_height % multiple
        dst_width = dst_width - dst_width % multiple
